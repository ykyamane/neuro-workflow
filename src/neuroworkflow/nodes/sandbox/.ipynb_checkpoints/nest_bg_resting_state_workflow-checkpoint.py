...
==============================================
Tests the 4 sandbox BG nodes end-to-end:

  NESTBGParametersNode  →  NESTBGNetworkNode
                        →  NESTBGSimulationNode
                        →  NESTBGWriterNode

Outputs are written to ./results/nest_bg/ (relative to where this script runs).

QUICK TEST   (~2 min):  scale_factor=1.0, sim_duration_ms=2000
FULL RUN     (~5 min):  sim_duration_ms=3000  (default, matches Girard et al.)

Usage
-----
  # From repo root, with NEST installed in the active environment:
  python src/neuroworkflow/nodes/sandbox/nest_bg_resting_state_workflow.py

  # Or inside a Jupyter notebook/JupyterHub kernel:
  %run src/neuroworkflow/nodes/sandbox/nest_bg_resting_state_workflow.py
...
from neuroworkflow.nodes.sandbox.NESTBGParametersNode import NESTBGParametersNode
from neuroworkflow.nodes.sandbox.NESTBGNetworkNode    import NESTBGNetworkNode
from neuroworkflow.nodes.sandbox.NESTBGSimulationNode import NESTBGSimulationNode
from neuroworkflow.nodes.sandbox.NESTBGWriterNode     import NESTBGWriterNode
...
params.configure(
    # Simulation timing
    sim_duration_ms = 2000.0,   # 2000 ms for quick test; use 3000 for full run
    warmup_ms       = 500.0,    # discard first 500 ms; use 1000 for full run
    dt_ms           = 0.1,
    ...
    # Population size (1.0 = published 1:834 scale, ~49 K neurons total)
    scale_factor = 1.0,
    ...
    # Output location
    output_dir = "./results/nest_bg",
)

writer.configure(output_dir="./results/nest_bg")
...
if success:
    ...
    print("   Results in: ./results/nest_bg/")
